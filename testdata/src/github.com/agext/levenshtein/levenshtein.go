...
func Match(str1, str2 string, p *Params) float64 {
	...
	if p.minScore > 0 {
		// if basic.minScore is lower than basic.bonusThreshold, we can use a simplified formula
		...
		if p.minScore < p.bonusThreshold {
			...
			// basic.minScore <= sim + basic.bonusPrefix*basic.bonusScale*(1-sim)
			// basic.minScore <= (1-dist/maxDist) + basic.bonusPrefix*basic.bonusScale*(1-(1-dist/maxDist))
			// basic.minScore <= 1 - dist/maxDist + basic.bonusPrefix*basic.bonusScale*dist/maxDist
			// 1 - basic.minScore >= dist/maxDist - basic.bonusPrefix*basic.bonusScale*dist/maxDist
			// (1-basic.minScore)*maxDist/(1-basic.bonusPrefix*basic.bonusScale) >= dist
